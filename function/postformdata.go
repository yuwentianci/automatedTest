...
func PostFormData(url string, formData map[string]interface{}, target interface{}) error {
	...
	req.Header.Set("api-version", "1001")
...
func PostFormDataManyToken(url string, formData map[string]interface{}, target interface{}, authorization string) error {
	// 准备表单数据
	body := &bytes.Buffer{}
	writer := multipart.NewWriter(body)
	...
	responseText, err := io.ReadAll(resp.Body)
	if err != nil {
		return errors.New("读取响应时发生错误: " + err.Error())
	}
